import {
  getClient,
  manageInitialPublishForPackage,
  Network,
...
const processSvg = () => {
  ...
  dataUrl = dataUrl
    .replace("REPLACE_ME_WITH_TEXTS", "{display.uri_encoded_name}")
...
export const publish = async (network: Network, clientConfigPath: string) => {
  ...
  publishPackage(
    txb,
    path.resolve(__dirname, "../../packages/package_info"),
    clientConfigPath
  );
  ...
  writeFileSync(
    path.resolve(__dirname + `/../package-info.${network}.json`),
...
export const setupPackageInfoDisplay = async (
