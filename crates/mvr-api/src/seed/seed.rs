...
use crate::AppState;
use mvr_schema::models::{NameRecord, Package, PackageInfo};
use mvr_schema::schema::{name_records, package_infos, packages};
use rand::Rng;
...
pub async fn seed_database(state: &AppState) -> Result<(), anyhow::Error> {
    let mut connection = state.reader().connect().await?;
    ...
    connection
        .execute(diesel::insert_into(package_infos::table).values(package_info_seeds))
        .await?;

    connection
        .execute(diesel::insert_into(packages::table).values(package_seeds))
        .await?;
    ...
    connection
        .execute(diesel::insert_into(name_records::table).values(name_record_seeds))
        .await?;
...
pub async fn load_seed_data(state: &AppState) -> Result<(), anyhow::Error> {
    ...
    connection
        .execute(diesel::sql_query(name_insert_query))
        .await?;
    ...
    let package_entries_query = format!(
        "INSERT INTO packages (package_id, original_id, package_version, move_package, chain_id, tx_hash, sender, timestamp) VALUES {}",
        package_entries.join(", ")
    );

    connection
        .execute(diesel::sql_query(package_entries_query))
        .await?;
    ...
    connection
        .execute(diesel::sql_query(package_infos_query))
        .await?;
