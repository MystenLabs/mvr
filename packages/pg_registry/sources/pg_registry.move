module pg_registry::pg_registry;
...
public struct PublicGoodName has key {
    id: UID,
    capability_id: ID,
}

public struct PublicGoodNameCap has key, store {
    id: UID,
    valid_for: ID,
}
...
/// Destroys the `PublicGoodName` and returns the NFT to the owner.
public fun destroy<T: store>(pg: PublicGoodName, cap: PublicGoodNameCap): T {
    let key = SuinsNftKey();
    assert!(pg.id.exists_with_type<_, T>(key), EInvalidType);
    assert!(cap.is_valid_for(&pg), EUnauthorized);

    let PublicGoodName { mut id, .. } = pg;

    let nft: T = id.remove(key);
    id.delete();
    let PublicGoodNameCap { id, .. } = cap;
    id.delete();

    nft
}

fun is_valid_for(cap: &PublicGoodNameCap, pg: &PublicGoodName): bool {
    cap.valid_for == pg.id.to_inner()
}

fun new(ctx: &mut TxContext): (PublicGoodName, PublicGoodNameCap) {
    let capability_id = object::new(ctx);
    let name = PublicGoodName {
        id: object::new(ctx),
        capability_id: capability_id.to_inner(),
    };

    let capability = PublicGoodNameCap {
        id: capability_id,
