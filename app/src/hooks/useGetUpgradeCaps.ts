...
const getUpgradeCaps = async (client: SuiClient, address: string) => {
    return fetchAllOwnedObjects({client, address, filter: {
...
const parseUpgradeCapContent = (cap?: SuiObjectResponse): UpgradeCap => {
    ...
    if (cap.data.content.dataType !== 'moveObject') throw new Error("Invalid upgrade cap object");
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (network === 'all') {
                ...
                return {
                    [network as Network]: (await getUpgradeCaps(clients[network as Network], address!) ?? [])
                };
            }
        ...
        select(data) {
            ...
            for (const key of Object.keys(data)) {
                ...
                res[key] = data[key as Network]?.map(x => parseUpgradeCapContent(x)) ?? [];
            }
